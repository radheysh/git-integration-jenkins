...
public class UtilityMethods {
	...
	public TrackTraceResponse handleException(Exception ex) {
		ex.printStackTrace();
		return new TrackTraceResponse(Constants.RESPONSE_CODE_500,"There are error while processing of request. Please contact admin with this uuid "+UUID.randomUUID().toString()+" "+ex.getMessage());
	}
	...
	public static void main (String aa[]) {
		...
		UtilityMethods.convertExcelDateToIso("06-Jan-2021");
		System.out.println(UtilityMethods.getCurrentDate4FileName());
	...
	public void handleError(String absoluteFilePath, String errorType) {
		...
		File rejectedFilePath = new File(Constants.USER_HOME+File.separator+Constants.FEED_FILES+File.separator+Constants.REJECTED);
		if(!rejectedFilePath.exists()) {
			rejectedFilePath.mkdirs();
		}
		new File(absoluteFilePath).renameTo
		(new File(rejectedFilePath.getAbsoluteFile()+File.separator+UUID.randomUUID().toString()+"_"+fileName));
